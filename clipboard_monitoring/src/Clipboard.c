...
static GdkClipboard* gtkclip = NULL;
...
static void _register_mimetypes( Clipboard* clipboard, const char* const* mimetypes, const int mimetype_count );
static void _unregister_mimetypes( Clipboard* clipboard, const char* const* mimetypes, const int mimetype_count );
...
static gboolean _parse_data( char*    stream,
                             gsize    size,
                             guint    mimetype_count,
                             char**   mimetype_array,
                             GBytes** data_array )
{
	guint stream_offset = 0;
	for ( int i = 0; i < mimetype_count; i++ ) {
		char* p = stream + stream_offset;

		char* lf_offset = g_utf8_strchr( p, size - stream_offset, '\r' );
		...
		*lf_offset = '\0';
		...
		char* sep_offset = g_utf8_strrchr( p, lf_offset - p, ';' );
		if ( sep_offset == NULL )
			return FALSE;
		*sep_offset = '\0';

		char* mimetype = p + 10;  // skip 'MimeType: '

		sep_offset += 18;  // skip '; Content-Length: '

		guint64 content_size = 0;
		if ( 1 != sscanf( sep_offset, "%" G_GUINT64_FORMAT, &content_size ) )
			return FALSE;

		lf_offset += 4;  // skip '\r\n\r\n'
		GBytes* bytes = g_bytes_new_static( lf_offset, content_size );

		p[ 0 ] = '\0';  // string end

		mimetype_array[ i ] = mimetype;
		data_array[ i ]     = bytes;

		stream_offset += ( lf_offset - p ) + content_size;
	}

	return TRUE;
}

static gboolean _on_data( GPollableInputStream* stream, Clipboard* clipboard )
{
	const int buffer_size = 100;
	char*     buffer      = g_new( char, buffer_size );
	GError*   error       = NULL;
	gssize    read_count  = g_input_stream_read( ( gpointer )stream, buffer, buffer_size - 1, NULL, &error );
	if ( error ) {
		goto fail2;
	}

	char* lf_offset = g_utf8_strchr( buffer, buffer_size, '\r' );
	if ( lf_offset == NULL )
		goto fail1;
	*lf_offset = '\0';
	...
	char* sep_offset = g_utf8_strchr( buffer, lf_offset - buffer, ';' );
	if ( sep_offset == NULL )
		goto fail1;
	*sep_offset = '\0';

	guint64 mimetype_count = 0;
	if ( 1 != sscanf( buffer + 16, "%" G_GUINT64_FORMAT, &mimetype_count ) )
		goto fail1;

	guint64 content_size = 0;
	if ( 1 != sscanf( sep_offset + 18, "%" G_GUINT64_FORMAT, &content_size ) )
		goto fail1;

	guint tail_size         = read_count - ( lf_offset + 4 - buffer );
	char* buffer2           = g_new( char, content_size + 1 );
	buffer2[ content_size ] = '\0';

	gint need_read_count = content_size - tail_size;
	if ( need_read_count > 0 ) {
		memcpy( buffer2, lf_offset + 4, tail_size );

		gsize _count = 0;
		g_input_stream_read_all( ( gpointer )stream, buffer2 + tail_size, need_read_count, &_count, NULL, NULL );
		if ( _count != need_read_count ) {
			g_warning( _( "读取的大小和需求不一致: %d %lu" ), need_read_count, read_count );
		}
	}
	else {
		memcpy( buffer2, lf_offset + 4, content_size );
	}

	char**   mimetype_array = g_new( char*, mimetype_count );
	GBytes** data_array     = g_new( GBytes*, mimetype_count );

	if ( _parse_data( buffer2, content_size, mimetype_count, mimetype_array, data_array ) ) {
		g_signal_emit( ( gpointer )clipboard, SIGNAL_notify, 0, mimetype_count, mimetype_array, data_array );
	}

	for ( int i = 0; i < mimetype_count; i++ ) {
		if ( data_array[ i ] )
			g_bytes_unref( data_array[ i ] );
	}
	g_free( data_array );
	g_free( mimetype_array );
...
static void _register_mimetypes( Clipboard* clipboard, const char* const* mimetypes, const int mimetype_count )
{
	g_return_if_fail( clipboard->connection );
	g_return_if_fail( clipboard->fifo_fd > 0 );

	GError* error = NULL;

	GUnixFDList* fd_list = g_unix_fd_list_new();
	g_unix_fd_list_append( fd_list, clipboard->fifo_fd, NULL );

	GVariantBuilder* builder = g_variant_builder_new( G_VARIANT_TYPE( "as" ) );
	for ( int i = 0; i < mimetype_count; i++ ) {
		g_variant_builder_add( builder, "s", mimetypes[ i ] );
	}
	GVariant* args = g_variant_new( "(as)", builder );
	g_variant_builder_unref( builder );

	g_dbus_connection_call_with_unix_fd_list( clipboard->connection,              // GDBusConnection
	                                          "io.github.q962",                   // Bus name
	                                          "/io/github/q962/ClipboardReport",  // Object path
	                                          "io.github.q962.ClipboardReport",   // Interface name
	                                          "register",                         // Method name
	                                          args,                               // Parameters (GVariant)
	                                          G_VARIANT_TYPE( "()" ),             // Expected response type
	                                          G_DBUS_CALL_FLAGS_NONE,             // Flags
	                                          3000,                               // Timeout (negative for default)
	                                          fd_list,
	                                          NULL,   // Cancellable
	                                          NULL,   // cb
	                                          &error  // Error
	...
	g_object_unref( ( gpointer )fd_list );
}
...
static void clipboard_init( Clipboard* self )
{
	...
	gtkclip             = gdk_display_get_clipboard( display );

#ifdef G_OS_UNIX

	GError* error = NULL;

	clipboard->connection = g_bus_get_sync( G_BUS_TYPE_SESSION, NULL, &error );
	if ( clipboard->connection == NULL ) {
		g_printerr( "Failed to connect to the session bus: %s\n", error->message );
		g_error_free( error );
		return;
	}
	g_object_weak_ref( ( gpointer )clipboard, ( gpointer )g_object_unref, clipboard->connection );

	g_mkdir_with_parents( app_share_path_translate( "data", NULL ), S_IRUSR | S_IWUSR );
	const char* fifo_path = app_share_path_translate( "/data/Clipboard.fifo", NULL );
	...
	clipboard->fifo_fd = fifo_fd;

	g_object_weak_ref( ( gpointer )clipboard, ( gpointer )close, GINT_TO_POINTER( fifo_fd ) );

	//////

	GInputStream* stream = g_unix_input_stream_new( fifo_fd, FALSE );
	if ( g_pollable_input_stream_can_poll( ( gpointer )stream ) ) {
		GSource* source = g_pollable_input_stream_create_source( ( gpointer )stream, NULL );
		g_source_set_callback( source, ( gpointer )_on_data, clipboard, NULL );
		g_source_attach( source, NULL );
	}

	g_object_weak_ref( ( gpointer )clipboard, ( gpointer )g_object_unref, stream );
#endif
...
void clipboard_register_mimetypes( Clipboard* clipboard, const char* const* mimetypes, const int mimetype_count )
{
	g_return_if_fail( CLIPBOARD_IS_( clipboard ) );
	g_return_if_fail( mimetypes != NULL );
	g_return_if_fail( mimetype_count != 0 );

	_register_mimetypes( clipboard, mimetypes, mimetype_count );
}

void clipboard_unregister_mimetypes( Clipboard* clipboard, const char* const* mimetypes, const int mimetype_count )
{
	g_return_if_fail( CLIPBOARD_IS_( clipboard ) );
	g_return_if_fail( mimetypes != NULL );
	g_return_if_fail( mimetype_count != 0 );

	_unregister_mimetypes( clipboard, mimetypes, mimetype_count );
}
