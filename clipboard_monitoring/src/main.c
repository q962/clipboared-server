...
{
	if ( luaL_dostring( LuaS, "require(\"gtkclip\").stop()" ) ) {
		const char* error_msg = lua_tostring( LuaS, -1 );
		fprintf( stderr, "Error _stop_server: %s\n", error_msg );
	}
...
static void activate( GtkApplication* application )
{
	...
	const char* mimetypes[] = { "text/plain;charset=utf-8", "image/png" };
	clipboard_register_mimetypes( clipboard, mimetypes, G_N_ELEMENTS( mimetypes ) );
	...
	gtk_window_set_title( ( gpointer )win, _( "剪切板服务管理" ) );
	...
	GtkLabel* ip_bind_label = ( gpointer )gtk_label_new( _( "IP 绑定" ) );
	...
	gtk_widget_set_tooltip_text( ( gpointer )alias_widget, _( "别名（可选），公布 mdns 服务时使用" ) );
	gtk_entry_set_placeholder_text( alias_widget, _( "别名" ) );
	...
	gtk_widget_set_tooltip_text( ( gpointer )port_widget, _( "设置端口号，为 0 时随机分配" ) );
	gtk_entry_set_placeholder_text( port_widget, _( "端口号" ) );
...
static void set_share_path()
{
	...
	char*       new_dirs             = g_strdup_printf( "%s%s%s",
                                      ...
                                      installed_share_path ? G_SEARCHPATH_SEPARATOR_S : "",
                                      installed_share_path ? installed_share_path : "" );
...
static gpointer _main( gpointer user_data )
{
	...
	setlocale( LC_ALL, "" );
	bindtextdomain( GETTEXT_PACKAGE, app_installed_share_path( "/locale", NULL ) );
...
static void _start_mdns( void* user_data )
{
	...
	char* str = g_strdup_printf(
	  _( "端口绑定：%s:%s\nWeb服务：http://%s:%s/web" ), arg->ip_str, arg->port, arg->ip_str, arg->port );
...
static int _error( lua_State* L )
{
	...
	char* str = g_strdup_printf( _( "错误：%s:%s" ), error_msg, error_code );
	_set_tip( str );
	g_free( str );
...
static int _data_prefix( lua_State* L )
{
	const char* path = lua_tostring( L, 1 );

	lua_pushstring( L, app_share_path_translate( path, NULL ) );
	return 1;
}
...
int luaopen_gtkclip( lua_State* L )
{
	...
	lua_pushstring( L, "data_prefix" );
	lua_pushcfunction( L, _data_prefix );
	lua_settable( L, -3 );
