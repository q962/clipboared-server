...
static int _clipdata_get( guint limit, guint count, struct ClipData* data )
{
	...
	if ( SQLITE_OK != sqlite3_bind_int( data_stmt, 1, limit ) ||
		...
		// gtk_label_set_label( msg_label, sqlite3_errmsg( db ) );
...
static void init_terigger( sqlite3* db )
{
	static const char* stmt_str = "DROP TRIGGER IF EXISTS list_auto_delete;"
	                              ...
	                              "    (SELECT COUNT(id) FROM list) > %u "
	                              "    AND "
	                              "    id IN (SELECT id FROM list ORDER BY id ASC LIMIT 1) ;"
	                              "END;";
...
struct ClipI* clipdata_manager_selected_do( GtkStack* stack )
{
	if ( g_once_init_enter_pointer( ( gpointer* )&db ) ) {
		GFile* file = g_file_new_build_filename( app_data_path_translate( "/data", NULL ), NULL );
		g_file_make_directory_with_parents( file, NULL, NULL );
		g_object_unref( file );

		char* db_path = g_strdup( app_data_path_translate( "/data/clipdata.db", NULL ) );
		...
		gtk_box_append( b1, ( gpointer )gtk_label_new( _( "行数限制: " ) ) );
